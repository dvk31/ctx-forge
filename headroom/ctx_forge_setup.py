"""ctx-forge context-tool support for Headroom.

Intended to be contributed upstream as ``headroom/ctx_forge/__init__.py`` so
that ``HEADROOM_CONTEXT_TOOL=ctx-forge`` becomes a valid selection alongside
``rtk`` and ``lean-ctx`` (see INTEGRATION.md in this folder for the wrap.py
diff). Stdlib-only on purpose: tomllib (3.11+) with a tomli fallback.
