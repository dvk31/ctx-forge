...
9. **The project's CI will lint `.ctx/` — and may be stricter than its hooks.** Django shops often run `ruff check .` in CI — the generated tools are in scope. Run the project's ruff config over `.ctx/` and fix before installing (observed: 16 violations blocked a commit on first dogfood — E501, UP038, E741). Do not trust a passing pre-commit hook as proof: hooks that verify only staged "code" paths can skip `.ctx/` entirely while CI's whole-tree `ruff check .` still fails it (observed: green hooks, red CI, 10 violations). Lint `.ctx/` with the project's config after *every* tool edit, not just at generation.
...
16. **INSTALLED_APPS defines the project; the filesystem lies.** A Django tree can carry entire app directories that are *not installed* — observed: a service whose `apps/` held 6 active apps plus ~16 copied, inactive app dirs from a sibling service (3x the active code, kept deliberately dormant; the host's own ruff config excluded them as dead weight). A naive `apps/**` surface indexes all of it, and `find`/`impact` answer confidently about apps that never run — data-not-source in a third costume. Scope the `[surface]` globs to an explicit list of active app dirs derived from INSTALLED_APPS (confirm against live `apps.get_app_configs()`), then make it durable with three locks: (a) `ctx regen` cross-checks the `[surface]` app dirs against the live INSTALLED_APPS and fails loudly on drift, so activating an app forces a surface update; (b) `impact` refuses targets outside the surface (exit 1 with a reason) instead of reporting a confident "0 importers" over files the staleness hash ignores; (c) adversarial goldens pin a symbol that exists *only* in the inactive copies (`not_contains` on its dir name, plus a "no matches" positive). Registry-backed commands (`schema`, `api`) get scoping for free — only INSTALLED_APPS models/routes exist after `django.setup()` — which makes them the ground-truth oracle for deriving the active list during the audit. Bonus closure check: an edit under an inactive copy must NOT trip `regen --check` (out of surface by design).
...
Author the questions as carefully as the tools — **the protocol catches question bugs too**. Observed: a question asserting `contains_line: "activate"` passed against output containing `/deactivate/` (substring match). For verb-like terms, anchor the assertion (full path segment, `path:line`, or a regex with boundaries) and consider a `not_contains` pinning the near-miss sibling. Second instance, opposite direction: a question pinned a decorated class to its `class` line from `rg`, but `inspect.getsourcelines` anchors a `@method_decorator`-wrapped class at the *decorator* line (the source block start) — the tool was right, the question was off by one. When deriving anchors for decorated classes/functions, expect the first decorator line, and record the post-mortem in `derived_from` either way.
