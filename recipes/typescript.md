...
Status: v0.1 · Applies to TS 4.x–5.x, Node 18+ · Validated by a full generate-and-verify run: an umbrella toolset over a five-package pnpm workspace (a CLI, a zod-based schema compiler, an MCP server, two auth SDKs) — 8 commands, 20/20 golden questions on the first regen, including the adversarial barrel/alias/dispatch cases below. The run corrected one pre-validation claim (see pitfall 2's workspace-specifier rule).
...
| `ctx impact` | module graph from per-file AST + two-tier resolution | Edge forms: static `import`, `export ... from` re-exports, dynamic `import()`, `require()` in `.cts`. Resolution is two-tier: relative specifiers via path probing that maps the NodeNext `./x.js` convention back to `x.ts`/`x.tsx`/`x/index.ts`; **workspace package specifiers via the workspace name map (package name -> package dir -> src entry), NOT `ts.resolveModuleName`** — see pitfall 2. Tag `import type` edges as type-only and include them (a type change has a type-radius). Affected tests = importers matching the project's test glob. Anchors are import-statement *start* lines (multi-line imports' closing `} from "..."` line is what grep finds — golden-question the difference). |
...
2. **Path aliases and workspace names defeat naive resolution — and `resolveModuleName` lies about workspace packages.** `@scope/pkg` does not exist on disk where a string-join puts it, but the compiler's own resolution is also wrong for impact purposes: it follows `package.json` `exports`, which points at `dist/` build output — your edge lands on an emitted file the surface rightly excludes. (Validated: the dogfood workspace's exports maps all target `./dist/index.js`.) Map workspace specifiers yourself: package name -> package dir (from the workspace manifest) -> `src/index.ts` (or the subpath under `src/`). Use `ts.resolveModuleName` only where tsconfig `paths` aliases point at *source*. Either way, never raw string-joins — `impact` is a confident liar otherwise (the Django recipe's pitfall 12 has the same shape: unresolved import forms report 0 importers).
